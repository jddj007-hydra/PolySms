...
| `Endpoint` | string | "sms.tencentcloudapi.com" | ❌ | API端点 |
| `OriginEndpoint` | string | null | ❌ | 用于签名计算的原始endpoint（内网代理场景） |
...
当使用内网代理转发腾讯云请求时，需要配置 `OriginEndpoint` 参数。
...
```json
{
  "TencentSms": {
    ...
    "SmsSdkAppId": "your-sms-sdk-app-id",
    "Region": "ap-beijing",
    "Endpoint": "internal-proxy.company.com",
    "OriginEndpoint": "sms.tencentcloudapi.com"
  }
}
```
...
builder.Services.AddPolySms(
    ...
    tencent => {
        ...
        tencent.SmsSdkAppId = "your-sms-sdk-app-id";
        tencent.Region = "ap-beijing";
        tencent.Endpoint = "internal-proxy.company.com";
        tencent.OriginEndpoint = "sms.tencentcloudapi.com";
    }
);
```

**工作原理**：
- `Endpoint`：实际发送HTTP请求的目标地址（内网代理服务器）
- `OriginEndpoint`：用于计算腾讯云TC3-HMAC-SHA256签名的原始endpoint
- 签名计算使用 `OriginEndpoint`，保证签名验证通过
- 实际请求发送到 `Endpoint` 配置的代理地址

**注意事项**：
- 不使用代理时，无需配置 `OriginEndpoint`
- `OriginEndpoint` 默认为 `null`，此时使用 `Endpoint` 进行签名计算
- 确保内网代理正确转发请求到腾讯云官方API
